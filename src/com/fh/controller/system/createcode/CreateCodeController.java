...
public class CreateCodeController extends BaseController {
	...
	public void proCode(HttpServletResponse response) throws Exception{
		...
		tabletop = null == tabletop?"":tabletop.toUpperCase();		//表名转大写
		String zindext = pd.getString("zindex");	   	   			//属性总数
		int zindex = 0;
		String objectNameUpper = pd.getString("field0").split(",")[0];
		if(null != zindext && !"".equals(zindext)){
			zindex = Integer.parseInt(zindext);
		}
		List<String[]> fieldList = new ArrayList<String[]>();   	//属性集合			========参数4
		for(int i=0; i< zindex; i++){
			String tmp[] = pd.getString("field"+i).split(",");
			if("是".equals(tmp[6])) {
				objectNameUpper = tmp[0];
			}
			fieldList.add(tmp);	//属性放到集合里面
		}
		Map<String,Object> root = new HashMap<String,Object>();		//创建数据模型
		root.put("fieldList", fieldList);
		root.put("TITLE", TITLE);
		root.put("AUTHOR", SecurityUtils.getSubject().getSession().getAttribute(Const.SESSION_USERNAME).toString());//说明
		root.put("packageName", packageName);						//包名
		root.put("objectName", objectName);							//类名
		root.put("objectNameLower", objectName.toLowerCase());		//类名(全小写)
		root.put("objectNameUpper", objectNameUpper);		        //主键
		...
		/*生成SQL脚本*/
		Freemarker.printFile("mysql_SQL_Template.ftl", root, "mysql数据库脚本/"+tabletop+".sql", filePath, ftlPath);
		Freemarker.printFile("oracle_SQL_Template.ftl", root, "oracle数据库脚本/"+tabletop+".sql", filePath, ftlPath);
