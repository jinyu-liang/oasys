...
<body class="no-skin">
							...
							<c:when test="${not empty varList}">
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										...
										<a href="<%=basePath%>uploadFiles/uploadImgs/${var.NAME}" title="${var.TITLE}" class="bwGal">
										 <img src="<%=basePath%>uploadFiles/uploadImgs/${var.NAME}" alt="${var.TITLE}" width="100" height="">
										</a>
										</c:if>
										</td>
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
											...
											&nbsp;
											<c:if test="${QX.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.PICTURES_ID}','${var.NAME}','${var.PATH}');" title="删除">
	...
	<script type="text/javascript">
		...
		//删除
		function del(Id,NAME,PATH){
			
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>pictures/delete.do?PICTURES_ID="+Id+"&PATH="+PATH+"&NAME="+NAME+"&tm="+new Date().getTime();
