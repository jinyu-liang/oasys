								...
								<c:when test="${not empty userList}">
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">${user.GROUP_NAME }（${user.REMARK }）</td>
											<td class="center">${user.ROLE_NAME }</td>
											<td class="center">${user.PHONE }</td>
											<td class="center">${user.PHONE1 }</td>
											<td class="center">${user.LC_IP}</td>
											<td class="center">${user.MAC}</td>
											<td class="center"><a title="发送电子邮件" style="text-decoration:none;cursor:pointer;" href="mailto:${user.EMAIL}" >${user.EMAIL }&nbsp;<i class="ace-icon fa fa-envelope-o"></i></a></td>
...
function toExcel(){
	...
	var url='<%=basePath%>user/excel.do?keywords='+keywords+'&ROLE_ID='+ROLE_ID;
