...
<body class="no-skin">
	<div class="main-container" id="main-container">
		...
		<div class="main-content">
			<div class="main-content-inner">
					<!-- 检索  -->
					<form action="monologue/list.do" method="post" name="Form" id="Form">
					...
					<table id="simple-table" class="table table-striped table-bordered table-hover"  style="margin-top:0px;">
						<thead>
							<tr>
								...
								<th class="center" style="width:200px;">主题</th>
								<th class="center" style="width:80px;">创建时间</th>
								<th class="center" style="width:50px;">状态</th>
								<th class="center" style="width:80px;">备注</th>
						...
						<c:choose>
							<c:when test="${not empty varList}" >
								...
								<c:forEach items="${varList}" var="var" varStatus="vs">
									<tr>
										<td class='center'>
											<label><input type='checkbox' name='ids' class="ace" value="${var.TAG_ID}" /><span class="lbl"></span></label>
										</td>
										<td class='center'>${vs.index+1}</td>
										<td class="center">${var.TAG_ID}</td>
										<td class="left">${var.TAG_NAME}</td>
										<td class="left">${var.P_NAME}</td>
										<td class="center">${var.START_DATE}</td>
										<td class="center"><c:if test="${var.STATUS==0}">有效</c:if><c:if test="${var.STATUS==1}">无效</c:if></td>
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
											...
											<c:if test="${QX.edit == 1 }">
											<a style="cursor:pointer;" class="green" onclick="edit('${var.TAG_ID}');" title="编辑">
												<i class="ace-icon fa fa-pencil bigger-130"></i>
											</a>
											</c:if>
											&nbsp;
											<c:if test="${QX.del == 1 }">
											<a style="cursor:pointer;" class="red" onclick="del('${var.TAG_ID}','${var.PATH}');" title="删除">
				...
				<table style="width:100%;">
					<tr>
						<td style="vertical-align:top;">
							...
							<c:if test="${QX.del == 1 }">
							<a class="btn btn-info btn-sm tooltip-info" onclick="addtop();">主题信息</a>
							</c:if>
						</td>
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>monologue/goAdd.do';
			 diag.Width = 600;
			 diag.Height = 490;
		...
		function addtop(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>monologue/goAddTop.do?P_CODE=800';
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if('${page.currentPage}' == '0'){
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>monologue/goEditU.do?TAG_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
			...
			if(confirm(msg)){ 
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>monologue/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
