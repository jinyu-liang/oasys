            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <!-- 检索  -->
                        <form action="breakdown/list.do" method="post" name="Form" id="Form">
                            ...
                            <table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">
                                ...
                                <tr>
                                    ...
                                    <th class="center">故障ID</th>
                                    <th class="center">故障标题</th>
                                    <th class="center">故障时间</th>
                                    <th class="center">故障处理人</th>
                                    <th class="center">故障报告人</th>
                                    <th class="center">所属子系统</th>
                                    <th class="center">故障状态</th>
                                    <th class="center">小组审核人员</th>
                                ...
                                <c:if test="${QX.cha == 1 }">
                                <c:forEach items="${allList}" var="var" varStatus="vs">
                                <tr>
                                    <td class='center'>
                                        <label><input type='checkbox' name='ids' class="ace" value="${var.BK_ID}" /><span class="lbl"></span></label>
                                    </td>
                                    <td class='center'>${vs.index+1}</td>
                                    <td class="center">${var.BK_ID}</td>
                                    <td class="cneter">
                                        <a href = "javascript:;" onclick="cha('${var.BK_ID}');">${var.BK_TITLE}</a>
                                    </td>
                                    <td class="center">${var.BK_TIME}</td>
                                    <td class="center">${var.BK_PROCESS_P}</td>
                                    <td class="center">${var.BK_REPORT_P}</td>
                                    <td class="center">${var.SUBSYS_NAME}</td>
                                    <td class="center">
                                        <c:if test="${var.BK_REP_STATUS==0}">故障报告登记</c:if>
                                        <c:if test="${var.BK_REP_STATUS==1}">提交审核</c:if>
                                        <c:if test="${var.BK_REP_STATUS==2}">审核不通过</c:if>
                                        <c:if test="${var.BK_REP_STATUS==3}">审核通过</c:if>
                                    </td>
                                    <td class="center">${var.BK_PANELAUDITOR }</td>
                                    <td class="center">
                                        <c:if test="${QX.edit != 1 && QX.del != 1 }">
                                            <span class="label label-large label-grey arrowed-in-right arrowed-in"><i class="ace-icon fa fa-lock" title="无权限"></i></span>
                                        </c:if>
                                        <c:if test="${QX.edit == 1 }">
                                            <a style="cursor:pointer;" class="green" onclick="edit('${var.BK_ID}');" title="编辑">
                                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                                            </a>
                                        </c:if>
                                        &nbsp;
                                        <c:if test="${QX.del == 1 }">
                                            <a style="cursor:pointer;" class="red" onclick="del('${var.BK_ID}');" title="删除">
...
<script type="text/javascript">
    ...
    function add() {
        ...
        diag.Drag=true;
        diag.Title ="新增";
        diag.URL = '<%=basePath%>breakdown/goAddB.do';
        diag.Width = 600;
        diag.Height = 490;
    ...
    function del(Id){
        if(confirm("确定要删除?")){
            top.jzts();
            var url = "<%=basePath%>breakdown/deleteB.do?BK_ID="+Id;
    ...
    function edit(Id){
        ...
        diag.Drag=true;
        diag.Title ="编辑";
        diag.URL = '<%=basePath%>breakdown/goEditB.do?BK_ID='+Id;
        diag.Width = 600;
        diag.Height = 465;
    ...
    function cha(Id){
        ...
        diag.Drag=true;
        diag.Title ="详细信息";
        diag.URL = '<%=basePath%>breakdown/gochaB.do?BK_ID='+Id;
        diag.Width = 600;
        diag.Height = 465;
